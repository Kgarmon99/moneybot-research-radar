...
<head>
    ...
    <style>
        ...
        /* Playbook Styles */
        .toolkit-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 20px; }
        .toolkit-card { background: #080808; border: 1px solid #333; padding: 25px; border-radius: 6px; position: relative; overflow: hidden; display: flex; flex-direction: column; }
        .toolkit-icon { width: 40px; height: 40px; background: #111; border: 1px solid #333; border-radius: 8px; display: flex; align-items: center; justify-content: center; margin-bottom: 15px; color: var(--fg); }
        .toolkit-title { margin: 0 0 10px 0; color: var(--fg); font-size: 1.2rem; }
        .toolkit-desc { margin: 0 0 20px 0; color: #aaa; font-size: 0.95rem; line-height: 1.5; flex-grow: 1; }
        .toolkit-btn { display: inline-flex; align-items: center; justify-content: center; background: var(--fg); color: var(--bg); padding: 10px 20px; border-radius: 4px; font-weight: 600; text-decoration: none; font-size: 0.9rem; transition: transform 0.2s; border: none; cursor: pointer; }
        .toolkit-btn:hover { transform: translateY(-2px); }
        .toolkit-btn-secondary { background: transparent; color: var(--fg); border: 1px solid #555; }
        .toolkit-btn-secondary:hover { background: #111; border-color: #888; }
        
        @media(max-width: 768px) { .toolkit-grid { grid-template-columns: 1fr; } }

        
        /* --- PRINT STYLES (Executive Handout) --- */
        @media print {
            body { background: #fff !important; color: #000 !important; padding: 0 !important; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif !important; }
            .tabs, .header-actions, .search-bar, .toolkit-grid, .momentum-grid, .footer { display: none !important; }
            h1, h2, h3, h4 { color: #000 !important; border-bottom: 1px solid #ccc !important; padding-bottom: 5px !important;}
            .container { max-width: 100% !important; margin: 0 !important; padding: 20px !important;}
            .card, .viz-card { background: #fff !important; border: 1px solid #ccc !important; box-shadow: none !important; margin-bottom: 20px !important; page-break-inside: avoid !important;}
            .state-badge { background: #fff !important; border: 1px solid #000 !important; color: #000 !important; }
            .state-badge.grade-A { background: #000 !important; color: #fff !important; border: 1px solid #000 !important; box-shadow: none !important; }
            .bar-fill { background: #000 !important; }
            .bar-track { background: #eee !important; border: 1px solid #ccc !important; }
            .tab-content { display: block !important; }
            #library, #pioneers, #feed, #timeline, #api, #blueprint, #playbook { display: none !important; }
...
<body>
    <div class="container">
        ...
        <div class="tabs">
            ...
            <button class="tab" onclick="switchTab('playbook')">Advocacy Toolkit</button>
        ...
        <div id="playbook" class="tab-content">
            <div style="margin-bottom: 25px;">
                <h2 style="font-size: 1.5rem; margin-bottom: 5px;">Advocacy Toolkit</h2>
                <p style="color: var(--accent); font-size: 0.95rem;">Standardized, empirically-backed assets for educators, advocates, and policymakers to drive institutional change.</p>
            </div>
            ...
            <div class="toolkit-grid">
                ...
                <div class="toolkit-card">
                    ...
                    <h3 class="toolkit-title">School Board Pitch Deck</h3>
                    <p class="toolkit-desc">A highly-visual, 10-slide presentation designed specifically for local school board meetings. It abstracts the macro Stanford and FINRA data into localized, undeniable arguments for curriculum adoption.</p>
                    <div style="display: flex; gap: 10px;">
                        <button onclick="window.print()" class="toolkit-btn" style="width: 100%;">Print Data Handout &darr;</button>
                    </div>
                </div>
                ...
                <div class="toolkit-card">
                    ...
                    <h3 class="toolkit-title">The Superintendent Memo</h3>
                    <p class="toolkit-desc">A 1-page executive brief addressing the three primary hurdles of deployment: Curriculum Sourcing, Teacher Credentialing, and Funding. Built to bypass institutional friction.</p>
                    <div style="display: flex; gap: 10px;">
                        <a href="data/financial_literacy_dataset.csv" download class="toolkit-btn toolkit-btn-secondary" style="width: 100%;">Download Dataset (CSV) &darr;</a>
                    </div>
                </div>
                ...
                <div class="toolkit-card">
                    ...
                    <h3 class="toolkit-title">Journalist Media Kit</h3>
                    <p class="toolkit-desc">High-resolution, unbranded statistical charts, raw demographic capability data, and foundational research citations. Everything required for journalists covering the student debt crisis.</p>
                    <div style="display: flex; gap: 10px;">
                        <button onclick="switchTab('api')" class="toolkit-btn toolkit-btn-secondary" style="width: 100%;">View Developer API &rarr;</button>
                    </div>
                </div>
                
                <!-- Email Template Engine -->
                <div class="toolkit-card" style="border-color: #00bbff;">
                    <div class="toolkit-icon" style="color: #00bbff; border-color: #00bbff;">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>
                    </div>
                    <h3 class="toolkit-title" style="color: #00bbff;">Representative Email Engine</h3>
                    <p class="toolkit-desc">For residents of "F" and "C" grade states: Generate an automatically pre-filled, data-backed email to your local state representative demanding a standalone graduation requirement.</p>
                    <div style="display: flex; gap: 10px;">
                        <a href="mailto:?subject=Urgent: Supporting a Standalone Financial Literacy Mandate&body=Dear Representative,%0D%0A%0D%0AThe data is clear: high school students who receive standalone financial education have significantly higher credit scores and lower delinquency rates as adults.%0D%0A%0D%0ACurrently, our state is failing to provide a guaranteed, standalone personal finance course for high school graduation. Meanwhile, 30 other states have already mandated this critical life skill.%0D%0A%0D%0AI strongly urge you to support legislation that upgrades our educational standards from embedded concepts to a guaranteed standalone mandate.%0D%0A%0D%0AYou can view the full national data and capability gaps here: https://kgarmon99.github.io/moneybot-research-radar/%0D%0A%0D%0ASincerely,%0D%0A[Your Name]" class="toolkit-btn" style="width: 100%; background: #00bbff; color: #000;">Draft Email Now &rarr;</a>
                    </div>
                </div>
